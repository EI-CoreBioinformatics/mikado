...
class NeighborTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(10, 110, 10):
            ...
            iv.insert(i, i+9)
            self.assertEqual(iv.right(Interval(i, i+1), num_intervals=1)[0].start, i)
        self.intervals = iv
    ...
    def test_left(self):
        ...
        self.assertEqual(str(iv.left(Interval(60, 61),
                                     num_intervals=2)), str([Interval(50, 59), Interval(40, 49)]))

        for i in range(10, 100, 10):
            r = iv.left(Interval(i, i+1), max_dist=10, num_intervals=1)
            self.assertEqual(r[0].end,  i - 1)

    def test_toomany(self):
        iv = self.intervals
        self.assertEqual(len(iv.left(Interval(60, 61), num_intervals=200)), 6)
    ...
    def test_right(self):
        ...
        self.assertEqual(iv.right(Interval(1, 2), max_dist=100, num_intervals=1)[0].start, 10)

        def get_right_start(b10):
            r = iv.right(Interval(b10, b10+1), num_intervals=1)
            assert len(r) == 1
            return r[0].start
        ...
        for i in range(0, 100, 10):
            r = iv.right(Interval(i-1, i), max_dist=10, num_intervals=1)
...
class LotsaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(0, self.max, 10):
            f = Interval(i, i)
            iv = iv.insert(f.start, f.end)

        for i in range(600):
            iv = iv.insert_interval(Interval(0, 1))
        self.intervals = iv

    def test_count(self):
        iv = self.intervals

        r = iv.right(1, num_intervals=33)
        self.assertEqual(len(r), 33)

        l = iv.left(1, num_intervals=33)
        self.assertEqual(len(l), 1)

        u = iv.right(1, num_intervals=9999)
        self.assertEqual(len(u), 250)

        # now increase max_dist
        u = iv.right(1, num_intervals=9999, max_dist=99999)
        self.assertEqual(len(u), 9999)

    def test_max_dist(self):
        iv = self.intervals
        r = iv.right(1, max_dist=0, num_intervals=10)
        self.assertEqual(len(r), 0)

        for n, d in enumerate(range(10, 1000, 10)):
            r = iv.right(1, max_dist=d, num_intervals=10000)
