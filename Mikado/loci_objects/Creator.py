...
def analyse_locus(slocus: Superlocus,
    ...
    sqllogger.setLevel(json_conf["log_settings"]["log_level"])
...
class Creator:
    ...
    def define_input(self):
        ...
        if self.input_file.endswith(".gtf"):
            parser = Mikado.parsers.GTF.GTF
        else:
            parser = Mikado.parsers.GFF.GFF3
    ...
    def setup_shm_db(self):
        ...
        self.main_logger.info("Copy into a SHM db: %s",
                              self.json_conf["run_options"]["shm"])
    ...
    def __print_gff_headers(self, locus_out, score_keys):
        ...
        if self.sub_out is not None:
            ...
            sub_metrics = csv.DictWriter(
                ...
                Mikado.loci_objects.superlocus.Superlocus.available_metrics,
                delimiter="\t")
            ...
            if row.is_exon is True:
                ...
                if current_transcript is not None:
                    ...
                    if test is False:
                        ...
                        raise Mikado.exceptions.UnsortedInput(error_msg)

                    if Mikado.loci_objects.superlocus.Superlocus.in_locus(
                        ...
                        if job is not None:
                            jobs.append(job)
                        current_locus = Mikado.loci_objects.superlocus.Superlocus(
                            current_transcript,
                            stranded=False,
                ...
                current_transcript = Mikado.loci_objects.transcript.Transcript(
                    row,
