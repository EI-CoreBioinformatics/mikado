...
class ConvertCheck(unittest.TestCase):
    ...
    def test_convert_from_bam(self):
        ...
        bam_inp = pkg_resources.resource_filename("tests", "test_mRNA.bam")
    ...
    def test_convert_from_problematic(self):
        probl = pkg_resources.resource_filename("tests", "Chrysemys_picta_bellii_problematic.gff3")
...
class PrepareCheck(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # cls.__genomefile__.write(pkg_resources.resource_stream("tests", "chr5.fas.gz").read())
        # cls.__genomefile__.flush()
        # cls.fai = pyfaidx.Fasta(cls.__genomefile__.name)
        cls.fai = pysam.FastaFile(pkg_resources.resource_filename("tests", "chr5.fas.gz"))
    ...
    def test_varying_max_intron(self):
        ...
        self.conf.prepare.files.gff = [pkg_resources.resource_filename("tests",
                                                                                test_file)]
        ...
        for cuff_file, test_file in itertools.product(
            ...
            for proc in (1, ):
                with self.subTest(test_file=test_file, cuff_file=cuff_file, proc=proc):
                    self.conf.prepare.files.gff[0] = pkg_resources.resource_filename("tests",
                                                                                              cuff_file)
                    self.conf.prepare.files.gff[1] = pkg_resources.resource_filename("tests",
                                                                                              test_file)
                    self.conf.prepare.files.out_fasta = "mikado_prepared.fasta"
                    self.conf.prepare.files.out = "mikado_prepared.gtf"
                    args.configuration = self.conf
    ...
    def test_prepare_with_cds(self):
        ...
        ann_gff3 = pkg_resources.resource_filename("tests", "annotation.gff3")
    ...
    def test_cdna_redundant_cds_not(self):
        ...
        gtf = pkg_resources.resource_filename("tests", "cds_test_1.gtf")
    ...
    def test_negative_cdna_redundant_cds_not(self):
        ...
        gtf = pkg_resources.resource_filename("tests", "cds_test_2.gtf")
        self.conf.prepare.files.gff = [gtf]
    ...
    def test_truncated_cds(self):
        ...
        files = [pkg_resources.resource_filename("tests", filename) for filename in files]
        ...
        self.conf.reference.genome = pkg_resources.resource_filename("tests",
...
class CompareCheck(unittest.TestCase):
    ...
    def test_index(self):
        ...
        for ref in files:
            with self.subTest(ref=ref), tempfile.TemporaryDirectory() as folder:
                ...
                os.symlink(pkg_resources.resource_filename("tests", ref), temp_ref)
    ...
    def test_compare_trinity(self):
        ...
        files = ["trinity.gtf",
                 ...
                 "trinity.bed12"]
        files = [pkg_resources.resource_filename("tests", filename) for filename in files]
        bam = pkg_resources.resource_filename("tests", "trinity.minimap2.bam")
    ...
    def test_compare_problematic(self):
        ...
        problematic = pkg_resources.resource_filename("tests", "Chrysemys_picta_bellii_problematic.gff3")
        namespace = Namespace(default=False)
        namespace.distance = 2000
        namespace.no_save_index = True
...
class CompareFusionCheck(unittest.TestCase):
    ...
    def test_false_fusion(self):
        ...
        ref_file = pkg_resources.resource_filename("tests", os.path.join("fusion_test",
                                                                                "fusion_test_ref.gff3"))
        pred_file = pkg_resources.resource_filename("tests", os.path.join("fusion_test",
...
class ConfigureCheck(unittest.TestCase):
    ...
    def test_mikado_config_daijin(self):
        ...
        namespace.scoring = None
        namespace.intron_range = None
        namespace.reference = pkg_resources.resource_filename("tests", "chr5.fas.gz")
        namespace.external = None
        namespace.mode = ["permissive"]
        namespace.threads = 1
        namespace.blast_targets = []
...
class PickTest(unittest.TestCase):
    ...
    def test_single_proc(self):
        ...
        self.configuration.db_settings.db = pkg_resources.resource_filename("tests", "mikado.db")

        self.configuration.pick.files.input = pkg_resources.resource_filename("tests",
                                                                                   "mikado_prepared.gtf")
        dir = tempfile.TemporaryDirectory()
    ...
    def test_subprocess(self):
        ...
        self.configuration.db_settings.db = str(pkg_resources.resource_filename("tests", "mikado.db"))
        self.configuration.log_settings.log_level = "WARNING"
        ...
        for num in (1, 2):
            with self.subTest(num=num):
                ...
                self.configuration.requirements.pop("compiled", None)
                self.configuration.as_requirements.pop("compiled", None)
    ...
    def test_different_scoring(self):
        ...
        self.configuration.pick.files.output_dir = os.path.abspath(dir.name)
        self.configuration.pick.files.input = pkg_resources.resource_filename("tests",
                                                                                   "mikado_prepared.gtf")
        ...
        shutil.copy(pkg_resources.resource_filename("tests", "mikado.db"),
        ...
        with open(os.path.join(self.configuration.pick.files.output_dir, "mikado.test_diff.loci.scores.tsv")) as tsv:
            ...
            score_names = [_ for _ in self.configuration.scoring]
    ...
    def test_different_scoring_2(self):
        ...
        self.configuration.pick.files.input = pkg_resources.resource_filename("tests", "mikado_prepared.gtf")
        ...
        self.configuration.db_settings.db = os.path.join(outdir.name, "mikado.db")
        self.configuration.pick.files.output_dir = os.path.join(outdir.name)
        json_file = os.path.join(outdir.name, "mikado.yaml")
        with open(json_file, "wt") as json_handle:
            print_config(self.configuration, json_handle, output_format="yaml")
        self.configuration.pick.files.output_dir = os.path.join(outdir.name)
        scoring_file = pkg_resources.resource_filename("tests", "scoring_only_cds.yaml")
...
class SerialiseChecker(unittest.TestCase):
    ...
    def test_subprocess_single(self):
        ...
        xml = pkg_resources.resource_filename("tests", "chunk-001-proteins.xml.gz")
        transcripts = pkg_resources.resource_filename("tests", "mikado_prepared.fasta")
        junctions = pkg_resources.resource_filename("tests", "junctions.bed")
        orfs = pkg_resources.resource_filename("tests", "transcripts.fasta.prodigal.gff3")
        uniprot = pkg_resources.resource_filename("tests", "uniprot_sprot_plants.fasta.gz")
    ...
    def test_xml_vs_tsv(self):
        xml = pkg_resources.resource_filename("tests", os.path.join("blast_data", "diamond.0.9.30.xml.gz"))
        tsv = pkg_resources.resource_filename("tests", os.path.join("blast_data", "diamond.0.9.30.tsv.gz"))
        queries = pkg_resources.resource_filename("tests", os.path.join("blast_data", "transcripts.fasta"))
        prots = pkg_resources.resource_filename("tests", "uniprot_sprot_plants.fasta.gz")
        logs = dict()
        dbs = dict()
    ...
    def test_serialise_external(self):
        ...
        base = pkg_resources.resource_filename("tests", "test_external")
...
class StatsTest(unittest.TestCase):
    ...
    def test_annotation_stats(self):
        ...
        annotation_file = pkg_resources.resource_filename("tests", "annotation.gff3")
        annotation_check = pkg_resources.resource_filename("tests", "annotation.gff3.stats")
    ...
    def test_stat(self):
        ...
        std_lines = []
        with pkg_resources.resource_stream("tests", "trinity_stats.txt") as t_stats:
            for line in t_stats:
    ...
    def test_problematic(self):
        ...
        files = [pkg_resources.resource_filename("tests", "Chrysemys_picta_bellii_problematic.gff3")]

        std_lines = []
        with pkg_resources.resource_stream("tests", "Chrysemys_picta_bellii_problematic.txt") as t_stats:
            for line in t_stats:
                std_lines.append(line.decode().rstrip())
...
class GrepTest(unittest.TestCase):
    ...
    @mark.slow
    def test_grep(self):
        files = [pkg_resources.resource_filename("tests", fname) for fname in ["trinity.gtf", "trinity.gff3"]]
        with io.TextIOWrapper(pkg_resources.resource_stream("tests", "trinity.ids")) as id_file:
    ...
    def test_problem_grep(self):
        fname = pkg_resources.resource_filename("tests", "Chrysemys_picta_bellii_problematic.gff3")
        flist = pkg_resources.resource_filename("tests", "Chrysemys_picta_bellii_problematic.list.txt")
        ...
        for flag in ("", "-v"):
            with self.subTest(flag=flag):
                ...
                if flag != "-v":
                    for line in pkg_resources.resource_stream("tests",
