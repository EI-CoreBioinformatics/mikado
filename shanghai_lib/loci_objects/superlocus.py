...
class superlocus(abstractlocus):
    ...
    #@profile   
    @asyncio.coroutine 
    def load_transcript_data(self, tid, loop=None ):
...
#         print("Loading data for {0}".format(tid))
        ...
        self.transcripts[tid].load_information_from_db( self.json_dict, introns = self.locus_verified_introns, session = self.session, loop=loop  )
