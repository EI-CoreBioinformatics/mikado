...
from Mikado.serializers.blast_serializer import Query, Hit
...
class Transcript:
    ...
    orf_baked += lambda q: q.filter(
        Mikado.serializers.orf.Orf.query == bindparam("query"))
    orf_baked += lambda q: q.filter(
        Mikado.serializers.orf.Orf.cds_len >= bindparam("cds_len"))
    ...
    def add_exon(self, gffline):
        ...
        if self.finalized is True:
            raise Mikado.exceptions.ModificationError(
                "You cannot add exons to a finalized transcript!")

        if self.id not in gffline.parent:
            raise Mikado.exceptions.InvalidTranscript(
                ...
                for exon in self.exons:
                    if exon in self.combined_cds:
                        ...
                        if len(self.combined_cds) == 1:
                            ...
                            raise Mikado.exceptions.InvalidCDS(
    ...
    def load_orfs(self, candidate_orfs):
        ...
        :type candidate_orfs: list[Mikado.parsers.serializers.orf.Orf
